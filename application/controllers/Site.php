...
class Site extends CI_Controller
{
    ...
    function contact()
    {
        $this->form_validation->set_rules('captcha', 'Captcha', 'trim|numeric');
        if ($this->form_validation->run() == FALSE) {

            $layout['keyword'] = "Best company for earning ";
            $layout['description'] = "Best company for earning ";
            $layout['title']="Connect Us";
            $captcha = captcha();
            ...
            if ($captcha_answer != $captcha_word) {
                $this->session->set_flashdata('site_flash', '<div class="alert alert-danger">Captcha not matches</div>');               
            ...
            $layout['keyword'] = "Best company for earning ";
            $layout['description'] = "Best company for earning ";
            $layout['title'] = "ThankYou";
            $name = $_POST['name'];
            $email = $_POST['email'];
            $subject = $_POST['subject'];
            $message = $_POST['message'];
            $to = config_item('info_email'); // Corrected email address
   ...
   <table>
       ...
       <tr>
            ...
            $user_headers = 'From: ' . $email . "\r\n";
            // $user_headers = 'From: ' . "\r\n";
            $user_message = "Dear $name.\nWelcome to " . config_item('company_name') . "\n We have got your query. We will reach you soon.\nThank You!";

            if (mail($to, $subject, $full_message, $headers)) {
                mail($user, $user_subject, $user_headers, $user_message);
                $data["response"] = "<h3>Dear <span style='color:#e70780;'>$name</span>,</h3><blockquote><p>We have got your query. We will contact you soon.<br/>For Quick Enquiry <span style='color:#e70780;'>Call Us</span> at <span><i class='fas fa-phone-alt px-1'></i><span> " . config_item('phone1') . "</span></span></p><p>Thank You!</p></blockquote>";
            } else {
                $data["response"] = "<h3>Dear <span style='color:#e70780;'>$name</span>,</h3><blockquote><p>Something is wrong. It seems like the internet is not working well.<br/>For Quick Enquiry <span style='color:#e70780;'>Call Us<span> at <span><i class='fas fa-phone-alt px-1'></i><span>" . config_item('phone1') . "</span></span></p><p>Please, try again!</p><p>Thank You!</p></blockquote>";
            }
    ...
    public function failed_registration()
    {
        if ($this->session->_user_id_ > 0) {
            ...
            $layout['title']="Registration Failed";
            $layout['keywords']="Best Company, Best Company in India";
            $layout['description']="Best Company, Best Company in India";            
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $captcah_answer = $this->input->post('captcha');
            $captcha_word = $this->input->post('captcha_word');

            if ($captcah_answer != $captcha_word) {
               
                $this->session->set_flashdata('site_flash', '<div class="alert alert-danger">Captcha Value Wrong</div>');
                redirect(site_url('site/login'));
            }
